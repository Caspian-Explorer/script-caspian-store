...
/**
 * Full ISO 3166-1 alpha-2 country list (249 entries), alpha-sorted by display
 * name. Single source of truth for every country dropdown in the library.
 *
 * Names are English only — translation is a separate concern handled by the
 * i18n layer if/when we decide to localize country names. Keeping names here
 * static avoids a 249 × N-locale explosion in the message table.
 */
export const ALL_COUNTRIES: readonly IsoCountry[] = [
...
/**
 * Build `<Select>` / `<SearchableSelect>` options from an `IsoCountry[]`.
 * Pass a `placeholder` as the first option (e.g. `{ value: '', label: '— Select —' }`)
 * when the dropdown needs an empty sentinel.
 */
export function toCountryOptions(
  list: readonly IsoCountry[] = ALL_COUNTRIES,
  placeholder?: SelectOption,
): SelectOption[] {
  const mapped = list.map((c) => ({ value: c.code, label: c.name }));
